...
//extern crate kvdb_rocksdb1 as kvdb_rocksdb;
...
use io::IoChannel;
use kvdb::KeyValueDB;
use kvdb_rocksdb1::{self, Database, DatabaseConfig};
...
pub fn restoration_db_handler(config: kvdb_rocksdb1::DatabaseConfig) -> Box<dyn BlockChainDBHandler> {
	struct RestorationDBHandler {
		config: kvdb_rocksdb1::DatabaseConfig,
	}
	...
	impl BlockChainDBHandler for RestorationDBHandler {
		fn open(&self, db_path: &Path) -> io::Result<Arc<dyn BlockChainDB>> {
			let key_value = Arc::new(kvdb_rocksdb1::Database::open(&self.config, &db_path.to_string_lossy())?);
